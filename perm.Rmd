...
In the case of testing for dependence between $y$ and $x$, under the null hypothesis: $f(y|x)=f(y)$.  
...
Different test statistics can produce the same p value. In this example, this is true for the t-statistic, the correlation, the covariance, and even the product $x'y$ (when the alternative hypothesis is one-sided). 
...
We want to test the equality of the distribution of `x` among the two (values in) `grp`:

$$H_{0x}:\ f(x|grp=0)=f(x|grp=1)$$ 
vs 

$$H_{1x}:\ f(x|grp=0)\neq f(x|grp=1)$$ 


This is, of course, equivalent to $f(x|grp)=f(x)$. We can use the mean difference as a test statistic; this is usually done in a parametric setting where, under the assumptions of normality and homoscedasticity, the only difference between the two groups can be in the means.
Conversely, within the permutation framework, the two samples can differ in any aspect of the distribution; adopting the difference in mean simply reflects our hope that this will produce a powerful test.


We formulate an analogous null hypothesis for `y.

$$H_{0y}:\ f(y|grp=0)=f(y|grp=1)$$ 
vs 
$$H_{1y}:\ f(y|grp=0)\neq f(y|grp=1)$$

We use `flip` to test for group differences (`grp`) on both `x` and `y`:
...
Here we are interested in testing the joint null hypothesis

$$H_{0}:\ H_{0x} \cap H_{0y}$$ 
vs

$$H_{1}:\ H_{1x} \cup H_{1y}$$ 
