...
In our example: $T=\hat\beta_1=\hat\sigma_{xy}/\hat\sigma_{xx}$ is the (estimated) slope.  
...
(see also Pesarin, 2001; [Hemerik \& Goeman, 2018](link.springer.com/content/pdf/10.1007/s11749-017-0571-1.pdf) )

Let $Y$ be data taking values in a sample space $\mathcal{Y}$. Let $\Pi$ be a -- usually finite -- set of transformations
...
**Theorem**: Under $H_0$, $E_P(\phi(Y))=\alpha$, that is $P(T(Y) > T^{(k)}) \leq \alpha$.

<!-- **Proof**   -->
<!-- By the group structure, $\pi\Pi = \Pi$ for all $\pi \in \Pi$. Hence, $T^{(k)}(\pi Y) = T^{(k)}(Y)$ for all $\pi \in \Pi$. Let $h$ have the uniform distribution on $\Pi$. Then, under $H_p$, the rejection probability is -->
<!-- $$ -->
<!-- P\{T(Y) > T^{(k)}(Y)\} = P\{T(hY) > T^{(k)}(hY)\} = P\{T(hY) > T^{(k)}(Y)\}. -->
<!-- $$ -->
<!-- The first equality follows from the null hypothesis, and the second equality holds since $T^{(k)}(Y) = T^{(k)}(hY)$. Since $h$ is uniform on $\Pi$, the above probability equals -->
<!-- $$ -->
<!-- \mathbb{E}_P\left[(\#\Pi)^{-1} \cdot \#\{\pi \in \Pi : T(\pi Y) > T^{(k)}(Y)\}\right] \leq \alpha, -->
<!-- $$ -->
<!-- as was to be shown. -->

**Proof**

For each $Y \in \mathcal{Y}$, define $O_y$ to be the orbit of $Y$, which is the set $\{\pi Y : \pi \in \Pi\} \subseteq \mathcal{Y}$.

Let $A = \{Y \in \mathcal{Y} : T(Y) > T^{(k)}(Y)\}$ be the set of elements of the sample space that lead to rejection. Suppose $H_p$ holds. By the group structure, $\Pi\pi = \Pi$ for all $\pi \in \Pi$. Consequently, $T^{(k)}(\pi Y) = T^{(k)}(Y)$ for all $\pi \in \Pi$. Thus, 
$$\#\{\pi   \in \Pi  : \pi Y \in A\} = \#\{\pi   \in \Pi  : T(\pi Y) > T^{(k)}(\pi Y)\} = \#\{\pi   \in \Pi  : T(\pi Y) > T^{(k)}(Y)\} \leq \alpha |O_Y|$$.

Endow the space of orbits with the $\sigma$-algebra that it inherits from the $\sigma$-algebra on $\mathcal{Y}$. Analogously to the proof of Theorem 15.2.2 in Lehmann (2005), we obtain
$$
...
**Randomization Hypothesis and Group structure**  

Randomization Hypothesis applied to $\pi \in \Pi$ having a Group structure implies 
$$
...
<!-- ```{r} -->
<!-- library(flip) -->
<!-- (res=flip(Reaction.Time~Age,data=reaction,perms=5000)) -->
<!-- ``` -->
