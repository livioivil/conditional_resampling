...
Over B=\ensuremath{10^{4}} permutations we got 9849 times a
...
\(| H_0)\) is equal to \(p =\) 0.0153, i.e.~very small.\\
...
\textbf{Proof}\\
By the group structure, \(\pi\Pi = \Pi\) for all \(\pi \in \Pi\). Hence,
\(T^{(k)}(\pi Y) = T^{(k)}(Y)\) for all \(\pi \in \Pi\). Let \(h\) have
the uniform distribution on \(\Pi\). Then, under \(H_p\), the rejection
probability is \[
P\{T(Y) > T^{(k)}(Y)\} = P\{T(hY) > T^{(k)}(hY)\} = P\{T(hY) > T^{(k)}(Y)\}.
\] The first equality follows from the null hypothesis, and the second
equality holds since \(T^{(k)}(Y) = T^{(k)}(hY)\). Since \(h\) is
uniform on \(\Pi\), the above probability equals \[
